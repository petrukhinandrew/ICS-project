...
class CentroidTracker:
	...
	def update(self, rects):
		...
		if len(self.objects) == 0:
			...
			centroids_pairwise_dists = dist.cdist(np.array(objects_centroids), input_centroids)
			...
			unused_rows = set(range(0, centroids_pairwise_dists.shape[0])).difference(used_rows)
			unused_cols = set(range(0, centroids_pairwise_dists.shape[1])).difference(used_cols)

			if centroids_pairwise_dists.shape[0] >= centroids_pairwise_dists.shape[1]:
				for row in unused_rows:
					objectID = objects_ids[row]
					self.disappeared[objectID] += 1
